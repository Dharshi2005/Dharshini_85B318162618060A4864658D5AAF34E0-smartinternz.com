class BankAccount:
  ...
  def deposit(self, amount):
    if amount > 0:
      ...
      print(
          f"{self.__account_holder_name} deposited ₹{amount}. New balance is ₹{self.__account_balance}"
      )
  ...
  def withdraw(self, amount):
    if 0 < amount <= self.__account_balance:
      self.__account_balance -= amount
      print(
          f"{self.__account_holder_name} withdrew ₹{amount}. New balance is ₹{self.__account_balance}"
      )
    else:
